              ...
              chrome.storage.local.get('stats', (data) => {
                ...
                if (stats) {
                  ...
                  const parsedDelList = replacementFiles.map(async (subId) => {
                    const bojData = await ufindData(subId);
...
function insertBoard(delList, token, hook) {
  ...
  notification += `업데이트 방식에 대한 요약은 다음과 같습니다. 자세한 내용은 패치 노트에서 확인 부탁드리며 꼭 숙지 후 업데이트를 실행하시길 바랍니다.</br>
                  <b>동의 후 실행</b>: 백준허브가 감지한 파일을 삭제 후 다시 제출합니다(일부 파일은 삭제되지 않을 수 있습니다).</br>
  ...
  yesButton.onclick = async () => {
    if (confirm('목록의 파일이 삭제되고 자동으로 업로드됩니다.\n진행하시겠습니까?')) {
      ...
      for (let idx = 0; idx < delList.length; idx++) {
        ...
        const result2 = await fetch(`https://api.github.com/repos/${hook}/contents/${elem.file2}`, {
          method: 'DELETE',
