...
browserAPI.storage.local.get('isSync', (data) => {
    ...
    if (!data || !data.isSync) {
        keys.forEach((key) => {
            browserAPI.storage.sync.get(key, (data) => {
                browserAPI.storage.local.set({ [key]: data[key] });
            });
        });
        browserAPI.storage.local.set({ isSync: true }, (data) => {
            // if (debug)
...
async function saveObjectInSyncStorage(obj) {
    return new Promise((resolve, reject) => {
        try {
            browserAPI.storage.sync.set(obj, function () {
                resolve();
            });
        } catch (ex) {
            reject(ex);
...
async function removeObjectFromSyncStorage(keys) {
    return new Promise((resolve, reject) => {
        try {
            browserAPI.storage.sync.remove(keys, function () {
                resolve();
            });
        } catch (ex) {
            reject(ex);
    ...
    return new Promise((resolve, reject) => {
        try {
            if (typeof chrome !== 'undefined' && chrome.storage) {
                ...
                reject(new Error('No browser storage API available'));
            }
        } catch (ex) {
            reject(ex);
        }
...
async function getOrgOption() {
    try {
        ...
        if (typeof chrome !== 'undefined' && chrome.storage) {
            ...
            browser.storage.local.set({ BaekjoonHub_OrgOption: "platform" });
        }
        return "platform";
    }
}
