...
async function parseData() {
  ...
  const result_message = [...document.querySelectorAll("#output > pre.console-content > div.console-message")].map(x => x.innerText).filter(x => x.includes(": ")).reduce((x, y) => x + '<br/>' + y);
...
async function makeData(origin) {
  ...
  // prettier-ignore
  const readme =
    `# [${division.replace("/", " > ")}] ${title} - ${problemId} \n\n`
    + `[문제 링크](${link}) \n\n`
    + `### 성능 요약\n\n`
    + `메모리: ${memory}, `
    + `시간: ${runtime}\n\n`
