...
const initializeEmptyRepoWelcome = async (token, hook, branch) => {
  ...
  const readmeContent = btoa(unescape(encodeURIComponent(`# ${repoName}\n\nAuto-generated by [BaekjoonHub](https://github.com/BaekjoonHub/BaekjoonHub)\n`)));
  // 1. blob
  const blob = await fetch(`https://api.github.com/repos/${hook}/git/blobs`, {
    method: 'POST', headers, body: JSON.stringify({ content: readmeContent, encoding: 'base64' }),
  }).then(r => r.json());
  // 2. tree (base_tree 없이)
  const tree = await fetch(`https://api.github.com/repos/${hook}/git/trees`, {
    method: 'POST', headers, body: JSON.stringify({ tree: [{ path: 'README.md', sha: blob.sha, mode: '100644', type: 'blob' }] }),
  }).then(r => r.json());
  // 3. commit (parents 없이)
  const commit = await fetch(`https://api.github.com/repos/${hook}/git/commits`, {
    method: 'POST', headers, body: JSON.stringify({ message: 'Initial commit - BaekjoonHub', tree: tree.sha, parents: [] }),
  }).then(r => r.json());
  // 4. ref 생성
  await fetch(`https://api.github.com/repos/${hook}/git/refs`, {
    method: 'POST', headers, body: JSON.stringify({ ref: `refs/heads/${branch}`, sha: commit.sha }),
  }).then(r => r.json());
};
    ...
    if (xhr.readyState === 4) {
      ...
      if (xhr.status === 200) {
        ...
        if (!bool) {
          ...
          // 빈 레포인 경우 초기 커밋 생성
          try {
            const refCheck = await fetch(`https://api.github.com/repos/${res.full_name}/git/refs/heads/${res.default_branch}`, {
              method: 'GET',
              headers: { Authorization: `token ${token}`, Accept: 'application/vnd.github.v3+json' },
            });
            if (refCheck.status === 409 || refCheck.status === 404) {
              // 빈 레포 — 초기화
              await initializeEmptyRepoWelcome(token, res.full_name, res.default_branch);
              console.log('Initialized empty repo with README.md');
            }
          } catch (e) {
            console.log('Empty repo check failed', e);
          }
