...
/* Get URL for welcome page - 브라우저 중립적인 방식으로 URL 생성 */
const welcomeURL = browserAPI.runtime.getURL('welcome.html');
    ...
    if (token === null || token === undefined) {
        ...
        xhr.addEventListener('readystatechange', function () {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    /* Show MAIN FEATURES */
                    browserAPI.storage.local.get('mode_type', (data2) => {
                        if (data2 && data2.mode_type === 'commit') {
                            $('#commit_mode').show();
                            /* Get problem stats and repo link */
                            browserAPI.storage.local.get(['stats', 'BaekjoonHub_hook'], (data3) => {
                    ...
                    browserAPI.storage.local.set({ BaekjoonHub_token: null }, () => {
 ...
 */
browserAPI.storage.local.get('bjhEnable', (data4) => {
    if (data4.bjhEnable === undefined) {
        $('#onffbox').prop('checked', true);
        browserAPI.storage.local.set({ 'bjhEnable': $('#onffbox').is(':checked') }, () => { });
    }
    else {
